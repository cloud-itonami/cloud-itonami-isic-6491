(ns leasing.portable-cljs-test-runner
  ...
  `leasing.store-contract-test`, which exercises the langchain.db
  ...
  -m if combined, hence -Sdeps for the extra path):
    ...
    clojure -Sdeps '{:paths [\"src\" \"test\"]}' \\
      ...
      --output-to target/tests.cjs -c leasing.portable-cljs-test-runner
  ...
  (:require [clojure.test :as t :refer [run-tests]]
            [leasing.facts-test]
            [leasing.governor-contract-test]
            [leasing.kernels.gate-test]
            [leasing.phase-test]
            [leasing.registry-test]
            [leasing.store-contract-test]))
