(ns leasing.operation
  "OperationActor -- one leasing operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (Leasing-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Leasing Governor (:govern) and the rollout phase
  ...
  One graph run = one leasing operation (intake -> advise -> govern ->
  ...
  decision to a human operator (the lessor officer). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:disbursement/fund` ALWAYS reaches this node when the governor is
  clean -- see `leasing.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [leasing.leasingllm :as leasingllm]
...
(defn build
  ...
  `leasing.store/Store`).
  opts:
    :advisor      -- a `leasing.leasingllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (leasingllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Leasing-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (leasingllm/-advise advisor store request)]
            {:proposal p :audit [(leasingllm/trace request p)]})))

      ;; Leasing Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
