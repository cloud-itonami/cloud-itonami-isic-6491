(ns leasing.kernels.gate
  "Safety kernel for the leasing governor + phase gate — the decision
  CORE of `leasing.governor/check` and `leasing.phase/gate`, extracted
  into the safe-kotoba subset (cloud-itonami kernels discipline,
  ADR-0016 / superproject ADR-2607101200), the leasing sibling of
  `cloud-itonami-isic-6511`'s `underwriting.kernels.gate` and
  `cloud-itonami-isic-6492`'s `credit.kernels.gate`.
  ...
  (`leasing.governor`, `leasing.phase`) reduce their inputs to
  ...
  Wire codes:
    ...
    coverage    collateral = the lease's :collateral-value, financed =
                its :financed-amount (same integer currency units). The
                minimum collateral-coverage ratio (1.0) is IN-KERNEL as
                minimum-coverage-ratio-x100 = 100 and decided EXACTLY
                in integers: insufficient iff
                100*collateral < minimum-coverage-ratio-x100*financed
                — no floating-point ratio ever enters the kernel. A
                non-positive financed amount reaching the kernel with
                the check applicable counts as insufficient
                (fail-closed) — the façade mirrors the registry's own
                guard and passes applicable=0 for missing/non-positive
                fields BEFORE the kernel, so façade behavior is
                unchanged.
    op          0 read (reserved — this actor has NO read ops; the
                façade never emits 0; kept for fleet-wide code parity)
                1 :lease/intake            2 :jurisdiction/assess
                3 :creditworthiness/screen 4 :disbursement/fund
  ...
  LESS autonomy (hold/escalate), never more. `:disbursement/fund`
  (op 4) is auto-enabled at NO phase — the same structural invariant
...
(def minimum-coverage-ratio-x100 100)

(defn coverage-insufficient
  "1 when the lease's collateral-coverage ratio falls short of the
  minimum coverage ratio. `applicable` is 0 when the op carries no
  coverage check or the lease's fields don't support one (norm-flag,
  fail-closed: any non-0 applies the check). The comparison
  (collateral/financed) < 100/100 is done EXACTLY in integer
  arithmetic as 100*collateral < 100*financed, so the kernel and the
  façade's rational/double ratio can never disagree at the boundary.
  Fail-closed range: a non-positive financed amount would bypass the
  ratio, so it counts as insufficient."
  [applicable collateral financed]
  (if (= (norm-flag applicable) 0)
    0
    (if (< 0 financed)
      (if (< (* 100 collateral) (* minimum-coverage-ratio-x100 financed)) 1 0)
      1)))

(defn hard-violation
  "1 when any HARD (human-un-overridable) violation is present:
  spec-basis missing / required leasing evidence incomplete /
  collateral coverage insufficient / adverse credit flag unresolved /
  lease funding already disbursed."
  [spec-missing evidence-incomplete cov-applicable cov-collateral
   cov-financed adverse-flag already-disbursed]
  (or5 (norm-flag spec-missing)
       (norm-flag evidence-incomplete)
       (coverage-insufficient cov-applicable cov-collateral cov-financed)
       (norm-flag adverse-flag)
       (norm-flag already-disbursed)))

(defn verdict-code
  "Governor verdict: 2 hard-hold wins over 1 escalate wins over 0 ok."
  [spec-missing evidence-incomplete cov-applicable cov-collateral
   cov-financed adverse-flag already-disbursed confidence-x100 actuation]
  (if (= 1 (hard-violation spec-missing evidence-incomplete cov-applicable
                           cov-collateral cov-financed adverse-flag
                           already-disbursed))
    2
...
(defn op-auto-enabled
  ...
  column). Exactly one cell is ever 1: phase 3 x :lease/intake.
  op 4 (:disbursement/fund) is 0 at every phase — permanent structural
  fact, not a rollout milestone. op 3 (:creditworthiness/screen) is
  likewise 0 at every phase."
  [phase op]
  (if (= phase 3) (if (= op 1) 1 0) 0))

(defn phase-disposition
  "Resolve the final disposition code from phase, op code and the
  governor's disposition code. Mirrors `leasing.phase/gate`: governor
...
(defn check-verdict [spec evid capp ccv cfa adv dbl conf act expected]
  (if (= (verdict-code spec evid capp ccv cfa adv dbl conf act) expected) 1 0))

(defn check-coverage [applicable collateral financed expected]
  (if (= (coverage-insufficient applicable collateral financed) expected) 1 0))
...
(defn battery-pass-count []
  (+
   ...
   (check-verdict 0 0 5 99999 100000 0 0 100 0 2)
   ;; -- coverage: exact-integer minimum-ratio boundary (100/100)
   (check-coverage 1 100000 100000 0)
   (check-coverage 1 99999 100000 1)
   (check-coverage 1 100001 100000 0)
   (check-coverage 1 120000 100000 0)
   (check-coverage 1 60000 100000 1)
   ;; -- coverage: not applicable / fail-closed ranges
   (check-coverage 0 60000 100000 0)
   (check-coverage 1 0 0 1)
   (check-coverage 1 100 -5 1)
   (check-coverage 1 0 1 1)
